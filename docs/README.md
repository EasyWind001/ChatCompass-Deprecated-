# 📚 ChatCompass 文档

本目录包含 ChatCompass 项目的详细技术文档。

## 📋 文档列表

### 核心文档

- **[PROJECT_SUMMARY.md](PROJECT_SUMMARY.md)** - 项目总体概述和架构设计
- **[search_implementation.md](search_implementation.md)** - 全文搜索实现细节

### 功能文档

- **[SEARCH_CONTEXT_FEATURE.md](SEARCH_CONTEXT_FEATURE.md)** - 搜索上下文定位功能文档
  - 功能说明
  - 技术实现
  - 使用示例
  - 配置参数

- **[SEARCH_ENHANCEMENT_SUMMARY.md](SEARCH_ENHANCEMENT_SUMMARY.md)** - 搜索增强功能总结报告
  - 实现对比
  - 效果演示
  - 测试结果

### 开发文档归档

`archive/` 目录包含开发过程中生成的各类文档，仅供参考：

- **功能开发报告**
  - FEATURE_SHOW.md - show命令功能文档
  - SHOW_FEATURE_SUMMARY.md - show功能总结

- **Bug修复报告**
  - BUGFIX_REPORT.md - ChatGPT爬虫修复报告
  - ISSUE_RESOLVED.md - 问题解决报告

- **测试报告**
  - TEST_REPORT.md - 完整测试报告
  - TEST_RESULTS.md - 测试结果
  - TESTING_COMPLETE.md - 测试完成报告
  - VERIFIED_WORKING.md - 功能验证报告

- **项目总结**
  - FINAL_REPORT.md - 最终项目报告
  - FINAL_TEST_SUCCESS.md - 测试成功报告
  - RUNNING_SUCCESS.md - 运行成功确认

- **快速指南**
  - START_HERE.md - 快速开始指南
  - QUICKSTART.md - 快速入门
  - 使用说明.txt - 中文使用说明
  - 搜索功能增强说明.txt - 搜索增强说明

- **发布公告**
  - NEW_FEATURE_SHOW.md - 新功能发布

## 📖 文档索引

### 按主题分类

#### 搜索功能
- [search_implementation.md](search_implementation.md) - 搜索实现原理
- [SEARCH_CONTEXT_FEATURE.md](SEARCH_CONTEXT_FEATURE.md) - 上下文搜索功能
- [SEARCH_ENHANCEMENT_SUMMARY.md](SEARCH_ENHANCEMENT_SUMMARY.md) - 搜索增强总结

#### 数据库设计
- [../database/schema.sql](../database/schema.sql) - 数据库表结构
- [PROJECT_SUMMARY.md](PROJECT_SUMMARY.md#数据库设计) - 数据库设计说明

#### 爬虫开发
- [archive/BUGFIX_REPORT.md](archive/BUGFIX_REPORT.md) - ChatGPT爬虫修复过程

#### 测试相关
- [../tests/README.md](../tests/README.md) - 测试套件说明
- [archive/TEST_REPORT.md](archive/TEST_REPORT.md) - 详细测试报告

## 🔗 相关链接

- [主README](../README.md) - 项目主文档
- [更新日志](../CHANGELOG.md) - 版本更新历史
- [测试说明](../tests/README.md) - 测试相关文档

## 📝 文档贡献

欢迎改进文档！提交文档更新时请：

1. 确保文档格式正确（Markdown）
2. 添加必要的代码示例
3. 更新本README的索引
4. 提交Pull Request

---

**最后更新**: 2026-01-13
